...
function main() {
  ...
  screens.forEach((fn, i) => {
    try {
      fn();
      figma.ui.postMessage({ type: 'progress', percent: Math.round((i + 1) / total * 100), text: `已生成 ${i + 1}/${total} 屏幕` });
    } catch (e) {
      figma.ui.postMessage({ type: 'progress', percent: Math.round((i + 1) / total * 100), text: `错误: ${e.message}` });
    }
  });
  
  figma.ui.postMessage({ type: 'done', text: `✅ 生成完成！共 ${total} 个屏幕。` });
